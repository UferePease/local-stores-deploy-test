...
function autocomplete(input, latInput, lngInput) {
    ...
    if (!input) {
        return; // skip this fxn from running if there is no input on the page
    }
...
function ajaxHeart(e) {
    ...
    // this takes an event that we will be hooking up to the submit event
    ...
    // submit with ajax
    _axios2.default.post(this.action) // remember, its a POST call. We are getting the url from this form's action attribute
    .then(function (res) {
        ...
        if (isHearted) {
            ...
            }, 2500); // using arrow fxn so we can reference 'this'
        }
    }).catch(console.error);
}
...
function makeMap(mapDiv) {
    ...
    // make our map
    var map = new google.maps.Map(mapDiv, mapOptions); // reference our map variable
    ...
    autocomplete.addListener('place_changed', function () {
        // using arrow fxn since we don't need to use 'this'
...
// this library helps sanitize the html that user provides

// function for displaying our search results
// const axios = require('axios');  // importing using traditional require
function searchResultsHTML(stores) {
    return stores.map(function (store) {
        // we are returning a link to each of the returned stores by mapping over each stores
...
// using es6 import cos we are being consistent with es6 on frontend
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/v1/search?q=' + this.value).then(function (res) {
            // we chain the response which is a promise
            if (res.data.length) {
...
(0, _autocomplete2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#long')); // NOTE: the $, and $ are made possible by the use of bling.js

(0, _typeAhead2.default)((0, _bling.$)('.search')); // call the typeAhead fxn

(0, _map2.default)((0, _bling.$)('#map')); // call the makeMap fxn
